...
async function searchMovies(query) {
    const response = await fetch(`https://api.themoviedb.org/3/discover/movie?api_key=${apiKey}&language=en-US&include_image_language=en&page=4&sort_by=popularity.desc`);
    const data = await response.json();
    return data;
}
...
async function displayResults4() {
    ...
    if (results.length === 0) {
        resultsContainer.innerHTML = '<p id="no">No results found.</p>';
        return;
    }
...
window.location.href="../pages/watch.html"; 
        ...
        const imageUrl = movie.poster_path 
            ? `https://image.tmdb.org/t/p/w500${movie.poster_path}` 
            : 'path/to/default-image.jpg';  // Default image if no poster is available
           movieElement.style.backgroundImage=`url("${imageUrl}")`;

        movieElement.innerHTML = `<h3 id="sp">${movie.title}</h3>`;
